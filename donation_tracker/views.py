...
def run(request,id):
  try:
    ...
    return tracker_response(request, 'donation_tracker/run.html', { 'event': event, 'run' : run, 'runners': runners, 'bids' : topLevelBids })
...
def prize(request,id):
  try:
    ...
    return tracker_response(request, 'donation_tracker/prize.html', { 'event': event, 'prize' : prize, 'games' : games,  'category': category })
